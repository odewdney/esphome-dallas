...
class ESPOneWire {
 public:
  ...
  /// Search for an alerting 1-Wire device on the bus. Returns 0 if all devices have been found.
  uint64_t active_search();

  // select device using search
  uint64_t search_select(uint64_t addr, uint8_t cmd);
 ...
 protected:
  ...
  uint64_t perform_search(uint8_t cmd);
